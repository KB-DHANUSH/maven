...
1. Server-side source code that will be generated using OpenApi tool based on an OpenApi specification document in the form
   ...
   target.generated-sources.server.src.main.java.group.server and will have the following components:
...
demonstration for both the OCI Monitoring and Logging SDK in src.main.java.group.server.GreetResource.java with simple annotations like below:
```java
    @Inject
...
As part of the example, some of the methods in src.main.java.group.server.GreetResource.java
...
src.main.java.group.server.GreetLivenessCheck.java and src.main.java.group.server.GreetReadinessCheck.java had been added
...
`SERVER_PORT` environment variable to the desired value:
   ```
   SERVER_PORT=8888 java -jar target/art-server.jar
   ``` 
...
1. Run the server application depending on the config profile. Below are the choices:
   * Default with no profile will use `config_file,instance_principals,resource_principal` authentication strategy
     ```bash
     java -jar target/art-server.jar
     ```
   * `test` profile will use user principal via oci config file authentication
     ```bash
     java -Dconfig.profile=test -jar target/art-server.jar
     ```
   * `prod` profile will use instance principal authentication
     ```bash
     java -Dconfig.profile=prod -jar target/art-server.jar
     ```
...
1. Build docker image
   ```
   docker build .. -t art-server -f Dockerfile
   ```
2. Run the application
   * Run from local machine using User Principal authentication. This will mount `~/.oci` as a volume in the running 
     docker container.
     ```
     docker run --user helidon --rm -p 8080:8080 -v ~/.oci:/home/helidon/.oci:ro art-server:latest
     ```
   * Run from an OCI instance using Instance Principal
     ```
     docker run --user helidon --rm -p 8080:8080  art-server:latest
     ```
...
[target/art-server-distribution.zip](target/art-server-distribution.zip)
...
```bash
$ scp target/art-server-distribution.zip user@DESTINATION_HOST:/home/user
```
3. Unzip (will extract to `art-server` directory) and run
```bash
$ unzip art-server-distribution.zip
$ java -jar art-server/art-server.jar
```